...
FILE* infile = stdin; // The file from which to read input lines

int main(int argc, char *argv[]) { // Breakpoint
    ...
    if (argc == 2 && strcmp(argv[1], "-") != 0) { // Breakpoint 
        infile = fopen(argv[1], "r"); // Open the file given as command line argument
        if (!infile) { // If the file cannot be opened
            printf("%s: Error: Couldn't open %s\n", argv[0], argv[1]); // Print error message
            exit(8); // Exit with error code 8
        }
    ...
    // b explode_bomb & b phase1
    char* input = read_line();       // Breakpoint, Get input 
